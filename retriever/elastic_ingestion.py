...
"""
Class to create elastic search index from document list.
Usage : PYTHONPATH=./ python agentic_bench/elastic_ingestion.py --input_passages api_before_rag.json --db_engine es-elser --actions i -I api-before-rag-dev --doc_based --server DEFINED_MT_RAG --max_doc_length 512 --stride 100 --output_file api_before_rag_indexed.json --data_type m3
"""

def setup_argparse():
    parser = ArgumentParser(description="Script to create/use ElasticSearch indices")
    parser.add_argument('--input_dir', '-dir',default=None,help="Directory containing files with passages. If set overrides input_passages.")
    ...
    parser.add_argument('--actions', default="ru",
    ...
    parser.add_argument("--data_type", default="auto", type=str, choices=["auto", 'pqa', 'sap', 'beir', 'rh', 'govt', 'm3','m3_multiturn'],
            ...
            for sent in parsed_text.sents:
                ...
                if slen > max_length:
                    ...
                    while len(too_long) > 0:
                        tl = too_long.pop()
                        ll = get_tokenized_length(tokenizer, text[tl[-1].idx : tl[0].idx + len(tl[0])])                        
                        # ll = get_tokenized_length(tokenizer, text[tl[-1].begin:tl[0].end])
                        if ll <= max_length:
                            tsizes.append(ll)
                            begins.append(tl[-1].idx)                         # start char of last token
                            ends.append(tl[0].idx + len(tl[0]))              # end char of first token                            
        ...
        with open(input_file) as in_file:
            if input_file.endswith(".tsv"):
                    ...
                    if 'answers' in row:
                        ...
                        passages.extend(
                            process_text(id=row['id'],
                                         title=remove_stopwords(fix_title(row['title']),
                ...
                if data_type in ['auto', 'sap']:
                    ...
                    txtname = "RAG_doc"
                    docidname = "sample_id"
                elif data_type == "m3_multiturn":
                    txtname = "doc"
                    docidname = "id"
                    data=m3_transform(data)
...
def m3_transform(data):
    """ Function to transform the dataset to extract documents and ids from M3.
    """
    doc_dict=[]
    doc_list=[]
    for item in data:
        sample_id=item["sample_id"]
        db_id=item["dataset_name"]
        for idx_turn, turn in enumerate(item["turns"]):
            for idx_hop, hop in enumerate(turn["gold_sequence"]):
                if "RAG" in hop["question_type"]:
                    for idx_doc, doc in enumerate(hop["rag_doc"]):
                        if doc not in doc_list:
                            doc_dict.append(
                                {
                                    "id": f"{db_id}_{sample_id}_{idx_turn}_{idx_hop}_{idx_doc}",
                                    "doc": doc,
                                }
                            )
                            doc_list.append(doc)
...
if __name__ == '__main__':
    ...
    if args.input_dir:
        ...
        args.input_passages=[os.path.join(args.input_dir,filename) for filename in filenames]
    ...
    if args.server == "SAP":
        ...
        client = Elasticsearch(
            "https://7f25eae6-4320-4daa-b5ce-58193a338255.974550db55eb4ec0983f023940bf637f.databases.appdomain.cloud:31575",
            ca_certs="/dccstor/arnaik_data/routing/agentic-middleware/agentic-middleware-playground/es_cert",
            basic_auth=("ibm_cloud_c0f25c4b_3f84_4586_9b52_38b9e4b9f637", ELASTIC_PASSWORD)
        )
        model_id = ".elser_model_2"
    ...
    if do_ingest or do_update:
        ...
        if args.db_engine in ["es-dense", 'es-bm25']:
            ...
            num_passages = len(input_passages)
            t = tqdm(total=num_passages, desc=f"Ingesting documents (w ELSER): {num_passages}", smoothing=0.05)
    ...
    if do_retrieve:
        ...
        if args.evaluate:
            input_queries = read_data(args.input_queries, fields=["id", "text", "relevant", "answers"],
                                      ...
                                      remove_stopwords=args.remove_stopwords, data_type=args.data_type,
                                      doc_based=doc_based_ingestion)
        else:
